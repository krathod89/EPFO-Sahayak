...
**Note on scope:** EPFO Findings confirms EPFiGMS has "a category dropdown + free-text box," but the source research did not capture the actual dropdown category names. This spec generates the free-text **body** only. Which dropdown category to pick is a gap — see Section 9, gap 8.
...
6. **The EPFiGMS category-dropdown taxonomy was not captured during research.** The source confirms EPFiGMS has "a category dropdown + free-text box," but not the actual category names. Section 6's grievance templates cover the free-text body only. A short audit of the live EPFiGMS form is needed before the grievance-generation flow can also select the correct dropdown category for the citizen.
