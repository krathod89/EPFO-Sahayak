...
- **Saved view:** same pattern as `feedback_submitted` above — Mixpanel's raw Events explorer (Data → Events), filtered to `codes_selected` where `codes` contains `CODE_10_UNLISTED_REASON`, columns narrowed to `Time` / `codes`. Created 2026-09-03, ahead of the first real external-traffic push (a Reddit post to r/EPFO), so rule-library coverage gaps are visible as soon as that traffic tests it, not discovered after the fact: `https://mixpanel.com/project/4058729/view/4555102/app/events#VkbX8ELDNQCh`. As of creation, lifetime volume was 2 of 36 `codes_selected` events (~5.5%) — under the 10% trigger below, but that's from pre-launch testing traffic, not real usage; re-baseline once Reddit traffic actually arrives.
